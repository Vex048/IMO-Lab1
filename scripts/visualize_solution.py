...
DEFAULT_RUNS = [
    {"tourfile": "outputs/nearest_neighbour_solution_cost.txt", "label": "Nearest Neighbour (cost)", "color": "tab:blue"},
    {"tourfile": "outputs/nearest_neighbour_solution_distance.txt", "label": "Nearest Neighbour (distance)", "color": "tab:orange"},
    {"tourfile": "outputs/weighted_two_regret.txt", "label": "Weighted Two-Regret", "color": "tab:green"},
]

DEFAULT_COMBINED_OUTPUT = "outputs/autoplay_regret.mp4"
DEFAULT_COLORS = [
    "tab:blue",
    "tab:orange",
    "tab:green",
...
def visualize_multi(dataset_path, runs, show=True, combined_save_path=None, interval_ms=500, fps=None, save_individual=False, save_dpi=DEFAULT_SAVE_DPI):
    ...
    if rewards:
        ...
        max_r = max(rewards)
        if max_r == min_r:
            node_sizes = [ (DEFAULT_NODE_SIZE_MIN + DEFAULT_NODE_SIZE_MAX) / 2.0 ] * len(rewards)
        else:
            node_sizes = [
                DEFAULT_NODE_SIZE_MIN + (r - min_r) / (max_r - min_r) * (DEFAULT_NODE_SIZE_MAX - DEFAULT_NODE_SIZE_MIN)
                for r in rewards
            ]
    else:
    ...
    for idx, r in enumerate(runs):
        if isinstance(r, (str, Path)):
            ...
            norm_runs.append({"tourfile": str(tourfile), "label": label, "color": color})
        else:
            tourfile = Path(r.get("tourfile"))
            label = r.get("label") or tourfile.stem
            color = r.get("color") or DEFAULT_COLORS[idx % len(DEFAULT_COLORS)]
            norm_runs.append({"tourfile": str(tourfile), "label": label, "color": color, "output": r.get("output")})
    ...
    range_x = maxx - minx
    range_y = maxy - miny
    padx = range_x * DEFAULT_PAD_FRAC * DEFAULT_SPACE_SCALE if range_x != 0 else DEFAULT_PAD_MIN * DEFAULT_SPACE_SCALE
    pady = range_y * DEFAULT_PAD_FRAC * DEFAULT_SPACE_SCALE if range_y != 0 else DEFAULT_PAD_MIN * DEFAULT_SPACE_SCALE

    label_offset = max(range_x, range_y) * DEFAULT_LABEL_OFFSET_FRAC * DEFAULT_SPACE_SCALE
    if label_offset == 0:
    ...
    def update(frame):
        ...
        title.set_text(f'Tour progress: {min(frame+1, max_len)}/{max_len} — Autoplay (multi)')
        return lcs

    anim = FuncAnimation(fig, update, frames=max_len, interval=interval_ms, blit=False, repeat=True)
    ...
    if combined_save_path:
        ...
        if ext == '.gif':
            if PillowWriter is None:
                raise SystemExit('PillowWriter not available; install pillow to save GIFs')
            use_fps = fps if fps is not None else max(1, int(round(1000.0 / interval_ms)))
            writer = PillowWriter(fps=use_fps)
        elif ext in ('.mp4', '.m4v'):
            if FFMpegWriter is None:
                raise SystemExit('FFMpegWriter not available; install ffmpeg to save MP4s')
            use_fps = fps if fps is not None else max(1, int(round(1000.0 / interval_ms)))
            ...
            if PillowWriter is not None:
                ...
                raise SystemExit('Unknown output format and Pillow not available to write GIFs')
    ...
    if save_individual:
        for idx, r in enumerate(norm_runs):
            ...
            anim2 = FuncAnimation(fig2, upd2, frames=len(segs), interval=interval_ms, blit=False, repeat=False)
            out2 = Path(run_out)
            ...
            if ext2 == '.gif':
                if PillowWriter is None:
                    ...
                    continue
                writer2 = PillowWriter(fps=fps if fps is not None else max(1, int(round(1000.0 / interval_ms))))
            elif ext2 in ('.mp4', '.m4v'):
                if FFMpegWriter is None:
                    print('Skipping per-run save (ffmpeg not available)')
                    continue
                writer2 = FFMpegWriter(fps=fps if fps is not None else max(1, int(round(1000.0 / interval_ms))))
            else:
                if PillowWriter is not None:
                    writer2 = PillowWriter(fps=fps if fps is not None else max(1, int(round(1000.0 / interval_ms))))
